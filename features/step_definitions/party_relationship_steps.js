...
defineSupportCode(function ({
	...
	Given('a party relationship of type {string} between party role {string} and party role {string} in status {string} with priority {string}', async function (party_relationship_type_description, from_party_role_description, to_party_role_description, party_relationship_status_type_description, priority_type_description) {
		this.priority_type                 = await this.db.one('select id, description, parent_id from priority_type where description=${priority_type_description}', {priority_type_description})
		...
		const input                        ={
			...
			priority_type_id                 : this.priority_type.id
		}
		this.party_relationship            =new PartyRelationship(await this.db.one('insert into party_relationship (comment, from_party_role_id, to_party_role_id, party_relationship_type_id, party_relationship_status_type_id, priority_type_id) values( ${comment}, ${from_party_role_id}, ${to_party_role_id}, ${party_relationship_type_id}, ${party_relationship_status_type_id}, ${priority_type_id}) returning id, comment,  from_party_role_id, to_party_role_id, party_relationship_status_type_id, party_relationship_type_id, priority_type_id', input))
	})
