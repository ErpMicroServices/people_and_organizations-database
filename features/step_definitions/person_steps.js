...
defineSupportCode(function ({
	...
	Given('I have provided a title of {stringInDoubleQuotes}', function (title, callback) {
		this.person.title = title;
		callback();
	});
	Given('I have provided a nickname of {stringInDoubleQuotes}', function (nickname, callback) {
		this.person.nickname = nickname;
		callback();
	});
	Given('I have provided a date of birth of {stringInDoubleQuotes}', function (date_of_birth, callback) {
		this.person.date_of_birth = date_of_birth ? moment(date_of_birth, "MM-DD-YYYY").toDate() : '';
		callback();
	});

	Given('I have made the comment that {stringInDoubleQuotes}', function (comment, callback) {
	...
	When('I update the first name to {stringInDoubleQuotes}', function (first_name) {
		return this.db
	...
	Then('the first name is {stringInDoubleQuotes}', function (first_name, callback) {
		expect(this.result.data.first_name).to.be.equal(first_name);
		callback();
	});

	Then('the last name is {stringInDoubleQuotes}', function (last_name, callback) {
		expect(this.result.data.last_name).to.be.equal(last_name);
		callback();
	});

	Then('the title is {stringInDoubleQuotes}', function (title, callback) {
		expect(this.result.data.title).to.be.equal(title);
		callback();
	});

	Then('the nickname is {stringInDoubleQuotes}', function (nickname, callback) {
		expect(this.result.data.nickname).to.be.equal(nickname);
		callback();
	});

	Then('the date of birth is {stringInDoubleQuotes}', function (date_of_birth, callback) {
		expect(this.result.data.date_of_birth.toString()).to.be.equal(moment(date_of_birth, "MM-DD-YYYY").toDate().toString());
		callback();
	});

	Then('the comment is {stringInDoubleQuotes}', function (comment, callback) {
