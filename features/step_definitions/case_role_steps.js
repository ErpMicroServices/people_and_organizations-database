...
defineSupportCode(function ({
	...
	Then('the roles include the parties', async function () {
		const parties    = this.case.roles.map(role => role.party_id)
		const case_roles = await this.db.any('select party_id from case_role where case_id = ${id}', this.case)
		case_roles.forEach(case_role => expect(case_role.party_id).to.be.oneOf(parties, `Expected ${case_role.toString()} to be in ${parties}`))
	})
